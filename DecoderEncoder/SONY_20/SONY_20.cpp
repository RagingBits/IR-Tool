// SONY_20.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
static std::string encode(int argc, char* argv[])
{
    ...
    if (argc == 3)
    {
        ...
        while (argc--)
        {
            ...
            {
                str = (argv[argc] + 9);
                extended = strtol(str.c_str(), end_str, 10);
        ...
        if ((address >= 0 && address <= 0x1F) && (command >= 0 && command <= 0x7F) && (extended >= 0 && extended <= 0xFF))
        {
    ...
    if (argc == 41)
    {
        ...
        if (VALID_VALUE(time_val, HEADER_ON_SIZE))
        {
            ...
            if (VALID_VALUE(time_val, OFF_SIZE))
            {
                ...
                extended = 0xFF & data;/*8bits*/
                data >>= 8;
                address = 0x1F & data;/*5bits*/
                data >>= 5;
                command = 0x7F & data;/*7bits*/

                reply = "command:" + std::to_string(command) + " address:" + std::to_string(address) + " extended:" + std::to_string(extended) + "\n\r";
